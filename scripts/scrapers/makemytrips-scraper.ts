...
export class MMTScraper extends BaseScraper {
  ...
  private generateMockHotels(city: string, country: string, isoCheckIn: string, isoCheckOut: string): any[] {
    ...
    const hotelNames: Record<string, string[]> = {
      'Mumbai': ['Oberoi Grand Mumbai', 'Hilton Mumbai', 'Novotel Mumbai Central', 'Crowne Plaza Mumbai', 'Ibis Mumbai'],
      'Goa': ['Sofitel Goa', 'Vivanta by Taj Goa', 'Radisson Goa', 'Club Mahindra Goa', 'Dunes Resort'],
      'Delhi': ['Shangri-La New Delhi', 'Le Meridien New Delhi', 'Grand Hyatt Delhi', 'The Claridges Delhi', 'Radisson New Delhi'],
      'Bangalore': ['St. Mark Hotel Bangalore', 'Citrine Hotel Bangalore', 'The Lalit Bangalore', 'Park Bangalore', 'Courtyard Bangalore'],
      'Dubai': ['Novotel Dubai Downtown', 'Radisson Dubai Downtown', 'Armani Hotel Dubai', 'Rove Downtown Dubai', 'Pullman Dubai Deira'],
    };

    const base = cityCoords[city] || [20, 77];
    const names = hotelNames[city] || ['MakeMyTrip Hotel', 'Journey Inn', 'Travel Lodge', 'Quest Hotel'];
  ...
  async scrape(params: { city: string; country: string; checkIn: Date; checkOut: Date }): Promise<ScrapeResult> {
    ...
    try {
      ...
      const hotels = this.generateMockHotels(city, country, isoCheckIn, isoCheckOut);
